...
> The entire creyzies NFT universe in a single git repository.

creyziebase is the entire creyzies NFT Universe stored in a single Git repository. It's made up of:
...
- Try the Demo: https://creyziebase.mixtape.network
...
With creyziebase, you can simply download the entire NFT universe through a single git clone and work with it 100% locally and offline. Use it in your app as often as you want, for free, forever!.
...
creyziebase indexes all properties of the NFT metadata inside a table named **metadata**, inside the [mixtape.db](mixtape.db) DB, and makes them queryable. Here are the attributes indexed:
...
creyziebase also lets you access all the associated IPFS files WITHOUT running an IPFS node!

Simply clone this git repository and it contains all the IPFS files for the creyziebase NFT Universe! You can check out all the files under [ipfs](ipfs).

## 3. WEB APP

creyziebase comes with a built-in query engine web app at [index.html](index.html)
...
You can try the web app at https://creyziebase.mixtape.network
...
```
git clone https://github.com/mixtape-network/creyziebase.git
```

## 2. Try the query web app

You can now run the entire creyziebase query engine web app on your local machine. **No network connection required since everything is on your machine**.

Simply go into the `creyziebase` folder you just downloaded, and run:
...
1. **Playing with the files:** Go into the creyziebase folder you just cloned, and play with all the files under the `ipfs` folder.
...
The built-in website lets you quickly query the DB at https://creyziebase.mixtape.network WITHOUT a server. Everything is loaded from the static page, **including the [mixtape.db](mixtape.db) DB.**
